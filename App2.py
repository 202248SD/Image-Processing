...
def sobel_edge(img, thresh=0):
    ...
    for x in range(WIDTH):
        for y in range(HEIGHT):
            ...
            edge_map[x, y] = G if G > thresh else 0
    return edge, gradient_direction


def sobel_gradient(edge, gradient_direction, thresh=100):
    ...
    for x in range(WIDTH):
        for y in range(HEIGHT):
            if gradient_direction[y][x] != 0 and edge_map[x, y] > thresh:
...
class ImageFilterApp:
    def __init__(self, root):
        ...
        self.slider_label = tk.Label(root, text="Adjust Sobel Threshold", bg=BACKGROUND_COLOR)
        self.slider_label.grid(row=2, column=1, padx=10, pady=0, sticky='sew')

        self.sobel_slider = tk.Scale(root, from_=0, to=255, orient="horizontal", bg=BACKGROUND_COLOR)
        self.sobel_slider.set(5)
        self.sobel_slider.grid(row=3, column=1, padx=5, pady=0, sticky='new')

        self.sobel_button = tk.Button(root, text="Apply Sobel", command=self.apply_sobel, bg=BUTTON_COLOR, fg=BUTTON_TEXT_COLOR)
        self.sobel_button.grid(row=4, column=1, padx=10, pady=5, sticky='ew')
        ...
        self.sobel_button_gradient.grid(row=5, column=1, padx=10, pady=5, sticky='ew')

        self.kernel_label = tk.Label(root, text="Choose Convolutions:", bg=BACKGROUND_COLOR, fg=FOREGROUND_COLOR)
        self.kernel_label.grid(row=6, column=1, padx=10, pady=5, sticky='w')

        self.kernel_vars = {}

        kernel_frame = tk.Frame(root, bg=BACKGROUND_COLOR)
        kernel_frame.grid(row=7, column=1, padx=10, pady=5, sticky='ew')
        ...
        for kernel_name in dir(kernels):
            if not kernel_name.startswith("__"):
                ...
                chk.grid(row=row_count, column=0, padx=5, pady=2, sticky='w')
        ...
        self.apply_button.grid(row=8, column=1, padx=10, pady=10, sticky='ew')
    ...
    def apply_sobel(self):
        if self.orig_image:
            ...
            self.filtered_image = sobel_edge(self.image, int(self.sobel_slider.get()))[0]
            self.display_image(self.filtered_image)

    def apply_sobel_gradient(self):
        if self.orig_image:
            self.image = intensity(self.orig_image)
            self.filtered_image = sobel_gradient(*sobel_edge(self.image, int(self.sobel_slider.get())))
            self.display_image(self.filtered_image)
